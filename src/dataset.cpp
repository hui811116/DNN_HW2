...
void Dataset::outputNumtoBin(int* outputVector, int vectorSize, mat& out)
{
	...
	out.resize(_numOfLabel,vectorSize);
	CCE(cudaMemcpy(out.getData(),tmpVector,sizeof(float)* vectorSize * _numOfLabel,cudaMemcpyHostToDevice));
	//mat outputMat(tmpVector, _numOfLabel, vectorSize);
	delete[] tmpVector;
	tmpVector = NULL;
}
void Dataset::inputFtreToMat(float** input, int r, int c, mat& out){
	...
	out.resize(r,c);
	CCE(cudaMemcpy(out.getData(),inputReshaped,sizeof(float)* r* c,cudaMemcpyHostToDevice));
